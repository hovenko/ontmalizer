...
public class XSD2OWLMapper {
	static final class RethrowingErrorHandler implements ErrorHandler {
		@Override
	...
	private void parseXSD(File file) {
		try {
			...
			parser.parse(file);
			parser.setErrorHandler(new RethrowingErrorHandler());
	...
	public void convertXSD2OWL() {
		...
		while (complexTypes.hasNext()) {
			XSComplexType xsComplexType = (XSComplexType) complexTypes.next();
			try {
	...
	private OntClass convertSimpleType(XSSimpleType simple, String parentURI) {
		...
		if (simple.isGlobal()) {
			...
			if (NS.equals(XSD.getURI())) {
				...
				OntClass dataType = ontology.createOntResource(OntClass.class, RDFS.Datatype, parentURI
						+ Constants.DATATYPE_SUFFIX);
	...
	private OntClass convertEnumeration(EnumeratedClass enumClass, XSSimpleType simple, SimpleTypeRestriction facets) {
		...
		if (baseNS.equals(XSD.getURI())) {
			enumClass.addSuperClass(ontology.createAllValuesFromRestriction(null, hasValue,
		...
		for (int i = 0, length = facets.enumeration.length; i < length; i++) {
			String memberURI = enumClass.getURI() + "_"
					+ facets.enumeration[i].replace('%', '_').replace('[', '_').replace(']', '_');
	...
	private OntClass convertComplexType(XSComplexType complex, String parentURI) {
		...
		if (complex.isGlobal()) {
			...
			throw new IllegalStateException("A ComplexType is either global or local: " + toS(complex));
		}

		final XSType baseType = complex.getBaseType();
		final String baseURI = getURI(baseType);
		...
		if (baseType.isSimpleType()) {
			if (ontology.getOntResource(baseURI) != null && !baseType.getTargetNamespace().equals(XSDDatatype.XSD)) {
				...
				if (baseType.getTargetNamespace().equals(XSDDatatype.XSD)) {
					AllValuesFromRestriction valuesFromRestriction = ontology.createAllValuesFromRestriction(null,
							hasValue, XSDUtil.getXSDResource(baseType.getName()));
					complexClass.addSuperClass(valuesFromRestriction);
				} else if (ontology.getOntResource(baseURI + Constants.DATATYPE_SUFFIX) != null) {
					AllValuesFromRestriction valuesFromRestriction = ontology.createAllValuesFromRestriction(null,
							hasValue, ontology.getOntResource(baseURI + Constants.DATATYPE_SUFFIX));
					complexClass.addSuperClass(valuesFromRestriction);
				}
			...
			if (complex.getDerivationMethod() == XSType.EXTENSION) {
				...
				/*
				 * If this complex type is a restriction to its base. Note that
				 * this part is slightly different from TopBraid. I added the
				 * allowed restrictions by getDeclaredAttributeUses below
				 */
				complexClass.addSuperClass(ontology.createClass(baseURI));
	...
	private void convertAttribute(XSAttributeUse attributeUse, OntClass complexClass) {
		 ...
		 * http://mail-archives.apache.org/mod_mbox/jena-users/201206.mbox/%3
		 * CCAFq2biyPYPKt0mnsnEajqwrQjOYH_geaFbVXvOuVeeDVYDgs2A
		 * 
		 * @mail.gmail.com%3E
		 */
		String attributeTypeName = attributeType.getName();
		if (NS.equals(XSDDatatype.XSD)) {
			if (attributeTypeName.equals("IDREFS") || attributeTypeName.equals("ENTITIES")
					|| attributeTypeName.equals("NMTOKENS")) {
				LOGGER.info("Skipping attribute type: {}", attributeTypeName);
				return;
			}
		}
			...
			if (term.isElementDecl()) {
				...
				if (maxOccurs == 1) {
					if (minOccurs == 1)
						...
						// minOccurs can be 0 in this case, logically
	...
	private RDFList getFacetList(SimpleTypeRestriction facets, XSRestrictionSimpleType restriction) {
		...
		if (facets.maxInclusive != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "maxInclusive"), facets.maxInclusive,
					XSDUtil.getXSDDatatype(name)));

		}
		if (facets.minInclusive != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "minInclusive"), facets.minInclusive,
					XSDUtil.getXSDDatatype(name)));
		}
		if (facets.maxExclusive != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "maxExclusive"), facets.maxExclusive,
					XSDUtil.getXSDDatatype(name)));
		}
		if (facets.minExclusive != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "minExclusive"), facets.minExclusive,
					XSDUtil.getXSDDatatype(name)));
		}
		...
		if (facets.totalDigits != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "totalDigits"), facets.totalDigits,
					XSDUtil.getXSDDatatype(name)));
		}
		if (facets.fractionDigits != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "fractionDigits"), facets.fractionDigits,
					XSDUtil.getXSDDatatype(name)));
		}
		if (facets.whiteSpace != null) {
			list = list.cons(ontology.createResource().addProperty(
					ontology.createProperty(XSD.getURI() + "whiteSpace"), facets.whiteSpace, XSDDatatype.XSDstring));
		}
